function mountRadial() {
  ...
  var zoom = d3v3.behavior
    .zoom()
    .scaleExtent([1, 10])
    .on("zoom", zoomed);

  var drag = d3v3.behavior
    .drag()
    .origin(function(d) {
      return d;
    })
  ...
  function updateForce(focusNode) {
    ...
    var simulation = d3v4
      ...
      .force("link", d3v4.forceLink(tree.links).strength(0.5));
  ...
  function dragged(d) {
    d3v3
      .select(this)
      .attr("cx", (d.x = d3v3.event.x))
      .attr("cy", (d.y = d3v3.event.y));
  }
